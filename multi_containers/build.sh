...
show_usage() {
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}Linux 内核统一构建脚本${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo ""
    echo -e "${YELLOW}用法:${NC}"
    echo "  $0 [选项]"
    echo ""
    echo -e "${YELLOW}选项:${NC}"
    echo "  -c, --config         更新内核配置"
    echo "  -k, --kernel         编译内核镜像"
    echo "  -m, --modules        编译内核模块"
    echo "  -a, --all            执行所有操作（config + kernel + modules）"
    echo "  -h, --help           显示此帮助信息"
    echo ""
    echo -e "${YELLOW}示例:${NC}"
    echo "  $0 --all             # 完整构建（配置 + 内核 + 模块）"
    echo "  $0 -k -m             # 只编译内核和模块（使用现有配置）"
    echo "  $0 --modules         # 只编译模块"
    echo "  $0 -c -m             # 重新配置并编译模块"
    echo ""
    echo -e "${YELLOW}环境变量:${NC}"
    echo "  ARCH                 目标架构（默认: arm64）"
    echo "  CROSS_COMPILE        交叉编译工具前缀"
    echo "  JOBS                 并行任务数（默认: CPU核心数）"
    echo "  OUTPUT_DIR           输出目录（默认: /output）"
    echo "  ROOTFS_DIR           Rootfs目录（默认: /output/rootfs）"
    echo "  FORCE_RECONFIG       强制重新配置（yes/no，默认: no）"
    echo "  COPY_DTB             是否复制DTB文件（yes/no，默认: no）"
    echo "  DEBUG_BUILD          调试构建模式（yes/no，默认: yes）"
    echo ""
}
...
COPY_DTB=${COPY_DTB:-no}
DEBUG_BUILD=${DEBUG_BUILD:-yes}
...
echo -e "${BLUE}执行操作:${NC}"
$DO_CONFIG && echo "  ✓ 更新内核配置"
$DO_KERNEL && echo "  ✓ 编译内核镜像"
$DO_MODULES && echo "  ✓ 编译内核模块"
echo ""
...
configure_kernel() {
    ...
    echo -e "${GREEN}✓ 配置文件已保存到 $OUTPUT_DIR/kernel.config${NC}"
    echo ""
}
...
build_kernel() {
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}[2/3] 编译内核镜像${NC}"
    echo -e "${GREEN}========================================${NC}"

    # 检查是否存在配置文件
    if [ ! -f ".config" ]; then
        echo -e "${RED}错误: 未找到 .config 文件${NC}"
        echo -e "${YELLOW}请先运行配置步骤: $0 --config${NC}"
        exit 1
    fi
...
build_modules() {
    ...
    # 清理旧的模块
    #echo -e "${YELLOW}清理旧的模块构建...${NC}"
    #make ARCH=$ARCH ${CROSS_COMPILE:+CROSS_COMPILE=$CROSS_COMPILE} modules_clean || true
...
if $DO_KERNEL || $DO_CONFIG; then
    echo -e "${YELLOW}内核文件:${NC}"
    ls -lh "$OUTPUT_DIR" 2>/dev/null | grep -v "^d" | grep -v "^total" | awk '{print "  " $9 " (" $5 ")"}' || true
    echo ""
fi
...
if $DO_MODULES; then
    if [ -d "$ROOTFS_DIR/lib/modules" ]; then
        ...
        if [ -n "$MODULE_VERSION" ]; then
            ...
            echo -e "  版本: $MODULE_VERSION"
            MODULE_COUNT=$(find "$ROOTFS_DIR/lib/modules/$MODULE_VERSION" -name "*.ko" 2>/dev/null | wc -l)
            echo -e "  数量: $MODULE_COUNT 个模块"
            MODULE_SIZE=$(du -sh "$ROOTFS_DIR/lib/modules/$MODULE_VERSION" 2>/dev/null | awk '{print $1}')
            echo -e "  大小: $MODULE_SIZE"
        fi
    fi
    echo ""
fi
